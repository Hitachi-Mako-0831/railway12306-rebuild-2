...
def read_passengers(
    ...
    crud.passenger.sync_default_for_user(db, current_user)
...
def create_passenger(
    *,
    ...
    existing = crud.passenger.get_by_user_and_id_card(
        ...
        id_type=passenger_in.id_type.value,
    ...
    *,
    ...
    if passenger.is_default:
        if (passenger_in.name and passenger_in.name != passenger.name) or (
            passenger_in.id_card and passenger_in.id_card != passenger.id_card
        ) or (
            ...
            raise HTTPException(
                status_code=400,
                detail=(
                    "Cannot modify name or ID info for the default passenger "
                    "(yourself)"
                ),
            )
    ...
    if passenger_in.id_card and passenger_in.id_card != passenger.id_card:
        existing = crud.passenger.get_by_user_and_id_card(
            ...
            user_id=current_user.id,
            id_type=(
                passenger_in.id_type.value
                if passenger_in.id_type
                else passenger.id_type
            ),
            id_card=passenger_in.id_card,
        )
        if existing:
...
def delete_passenger(
    *,
    ...
    if passenger.is_default:
        raise HTTPException(
            status_code=400,
            detail="Cannot delete default passenger (yourself)",
        )
